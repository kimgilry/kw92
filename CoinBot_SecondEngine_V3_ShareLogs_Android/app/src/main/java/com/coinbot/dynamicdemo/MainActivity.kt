...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUi(): ScrollView {
        ...
        val shareBtn = button("솔에게 기록 보내기") { shareTradingLog() }
        val clearBtn = button("기록 초기화") {
            startService(Intent(this, MarketService::class.java).setAction(MarketService.ACTION_CLEAR_LOG))
            Toast.makeText(this, "기록 초기화 요청 완료", Toast.LENGTH_SHORT).show()
        }
        val shareRow = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }
        shareRow.addView(shareBtn, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f))
        shareRow.addView(clearBtn, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 0.55f))
    ...
    private fun shareTradingLog() {
        try {
            val src = File(filesDir, "coinbot_history.csv")
            if (!src.exists() || src.length() == 0L) {
                Toast.makeText(this, "아직 보낼 기록이 없습니다.", Toast.LENGTH_SHORT).show()
                return
            }

            val shareDir = File(cacheDir, "shared").apply { mkdirs() }
            val stamp = java.text.SimpleDateFormat("yyyyMMdd_HHmmss", Locale.KOREA).format(java.util.Date())
            val out = File(shareDir, "CoinBot_Record_$stamp.csv")
            ...
            val send = Intent(Intent.ACTION_SEND).apply {
                ...
                putExtra(
                    Intent.EXTRA_TEXT,
                    "CoinBot 모의투자 기록입니다. BTC/ETH 초단위 엔진의 진입·HOLD·추가매수·익절과 점수/목표비중을 분석해주세요."
                )
                addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }
            startActivity(Intent.createChooser(send, "기록을 보낼 앱 선택"))
        } catch (e: Exception) {
            Toast.makeText(this, "기록 공유 실패: ${e.message}", Toast.LENGTH_LONG).show()
        }
    }
